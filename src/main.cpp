...
class $modify(GJAccountSettingsLayer)
{
	...
	bool init(int a1)
	{
		...
		label->setPositionY(label->getPositionY() + 111);
		label->setScale(0.55);
		BG->addChild(label);
		auto elmenu = CCMenu::create();
		elmenu->setPosition(winSize / 2);
		elmenu->setPositionX(elmenu->getPositionX()-276);
		elmenu->setPositionY(elmenu->getPositionY() + 120);
		...
		waexd2->setPositionY(waexd2->getPositionY() -55);
		...
		BG->addChild(optionsmenu, 2);
		BG->addChild(elmenu,2);
		BG->runAction(spawn);
...
#include <cstdio>
    ...
    //SOME SHIT FOR GEODE 
		...
		if (this->m_uReference == 3 && this->m_width <= 5) {
			CommentCell::draw();
		}
